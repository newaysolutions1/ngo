...
function DonationForm({ onSuccess, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-y-4 gap-x-6 sm:grid-cols-[auto_1fr]">
      ...
      <Field id="amount" type="number" placeholder="1000" {...commonProps("amount")} />

      <label className="self-center text-sm font-medium text-gray-700">Name</label>
      <Field id="name" type="text" placeholder="John Doe" {...commonProps("name")} />

      <label className="self-center text-sm font-medium text-gray-700">Email&nbsp;ID</label>
      <Field id="email" type="email" placeholder="john@example.com" {...commonProps("email")} />

      <label className="self-center text-sm font-medium text-gray-700">Phone&nbsp;Number</label>
      <Field id="phone" type="tel" placeholder="9876543210" {...commonProps("phone")} />
  ...
  return (
    <div className="mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {donateOpen && (
        ...
        >
          ...
          >
            ...
            <DonationForm onSuccess={() => setDonateOpen(false)} onCancel={() => setDonateOpen(false)} />
          </div>
        </div>
      )}
      ...
      <div className="py-20 px-4 sm:px-6 lg:px-12">
        <h1 className="mb-10 text-center text-4xl font-bold">Our Case Studies</h1>
        <Swiper
      ...
      {open && (
        <Fragment>
          <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onClick={() => setOpen(null)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="relative flex h-[88vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
              ...
              <section className="flex flex-col gap-8 p-8 lg:flex-row">
                <div className="w-full rounded-md border border-black/10 lg:w-[40%]">
                  <img src={open.image} alt={open.name} className="h-60 w-full rounded-xl object-cover blur-sm lg:h-72" />
                </div>
              ...
              <section className="flex-1 overflow-y-auto px-6 pb-8">
                ...
                <p className="whitespace-pre-line leading-relaxed text-gray-600">{open.story}</p>
              ...
              <button
                ...
                className="absolute top-2 right-3 rounded-full text-2xl text-gray-500 hover:bg-gray-100 hover:text-black"
              >
                ×
              </button>
            </div>
